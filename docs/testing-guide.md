...
PR/`dev`/`main` CI runs Ubuntu fmt, locked workspace Clippy/tests (including PostgreSQL), `cargo audit`, a debug CLI build/help check, installer dry runs, and immutable-workflow-reference checks. A separate native credential-store matrix runs store/config regressions and Tokio-hosted native save/reopen, missing-key, and empty-store recovery checks on Linux, Windows, macOS ARM64, and macOS Intel. Linux also checks fail-closed behavior against a locked Secret Service and uses `scripts/test-linux-keyring-compatibility.sh` to prove the synchronous and asynchronous providers can each read a shared key and encrypted database created through the other. These native operations have explicit command or job deadlines. Releases call both workflows for the exact source SHA and require them, plus every extracted-package smoke check, before publication. Merely adding the jobs does not establish that a particular commit passed them; retain the hosted run evidence before release.
